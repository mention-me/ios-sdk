...
class FindReferrerResultsViewController: UIViewController {
  ...
  func configureUI() {
    ...
    getOffButton.addTarget(
      self, action: #selector(getOffAction), for: UIControl.Event.touchUpInside)
    ...
    if let summary = referrer?.offer?.refereeReward?.summary {
      ...
      label2.text =
        "Congratulations! Because \(firstname) referred you, you've got our BEST introductory offer... Simply enter your email address to get your voucher."
    }
  ...
  @objc func getOffAction() {
    ...
    let referrerParameters = MentionmeReferrerParameters(
      referrerMentionMeIdentifier: identifier, referrerToken: token)
    //Create the Customer Parameters
    let customerParameters = MentionmeCustomerParameters(
      emailAddress: email, firstname: "", surname: "")
    //Create the Referee Register Request
    let request = MentionmeRefereeRegisterRequest(
      mentionmeReferrerParameters: referrerParameters,
      mentionmeCustomerParameters: customerParameters)
  ...
  func prepareToNavigateToLinkCustomerResultScreen(
    offer: MentionmeOffer?, refereeReward: MentionmeRefereeReward?,
    contentCollectionLink: MentionmeContentCollectionLink?, status: String?, email: String
  ) -> [String: Any?] {
    let dict: [String: Any?] = [
      "offer": offer, "refereeReward": refereeReward,
      "contentCollectionLink": contentCollectionLink, "status": status, "email": email,
    ]
    return dict
  }
  ...
  override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
    ...
    if let destination = segue.destination as? LinkCustomerViewController,

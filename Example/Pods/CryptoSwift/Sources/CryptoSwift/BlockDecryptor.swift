...
public class BlockDecryptor: Cryptor, Updatable {
  ...
  public func update(withBytes bytes: ArraySlice<UInt8>, isLast: Bool = false) throws -> [UInt8] {
    ...
    if worker.additionalBufferSize > 0 {
      ...
      accumulatedWithoutSuffix = Array(
        accumulated.prefix(accumulated.count - worker.additionalBufferSize))
    } else {
    ...
    for var chunk in accumulatedWithoutSuffix.batched(by: blockSize) {
      if isLast || (accumulatedWithoutSuffix.count - processedBytesCount) >= blockSize {
        ...
        if isLast, isLastChunk, var finalizingWorker = worker as? FinalizingDecryptModeWorker {
          chunk = try finalizingWorker.willDecryptLast(
            bytes: chunk + accumulated.suffix(worker.additionalBufferSize))  // tag size
        }

...
public struct CCM: StreamMode {
  ...
  public init(
    iv: [UInt8], tagLength: Int, messageLength: Int, additionalAuthenticatedData: [UInt8]? = nil
  ) {
    self.nonce = iv
  ...
  ///   - additionalAuthenticatedData: Additional authenticated data.
  public init(
    iv: [UInt8], tagLength: Int, messageLength: Int, authenticationTag: [UInt8],
    additionalAuthenticatedData: [UInt8]? = nil
  ) {
    self.init(
      iv: iv, tagLength: tagLength, messageLength: messageLength,
      additionalAuthenticatedData: additionalAuthenticatedData)
    self.authenticationTag = authenticationTag
  }

  public func worker(blockSize: Int, cipherOperation: @escaping CipherOperationOnBlock) throws
    -> CipherModeWorker
  {
    if nonce.isEmpty {
    ...
    return CCMModeWorker(
      blockSize: blockSize, nonce: nonce.slice, messageLength: messageLength,
      additionalAuthenticatedData: additionalAuthenticatedData, tagLength: tagLength,
      cipherOperation: cipherOperation)
  }
}

class CCMModeWorker: StreamModeWorker, SeekableModeWorker, CounterModeWorker,
  FinalizingEncryptModeWorker, FinalizingDecryptModeWorker
{
  ...
  init(
    blockSize: Int, nonce: ArraySlice<UInt8>, messageLength: Int,
    additionalAuthenticatedData: [UInt8]?, expectedTag: [UInt8]? = nil, tagLength: Int,
    cipherOperation: @escaping CipherOperationOnBlock
  ) {
    ...
    let hasAssociatedData =
      additionalAuthenticatedData != nil && !additionalAuthenticatedData!.isEmpty
    processControlInformation(
      nonce: self.nonce, tagLength: tagLength, hasAssociatedData: hasAssociatedData)
  ...
  private func processControlInformation(nonce: [UInt8], tagLength: Int, hasAssociatedData: Bool) {
    let block0 = try! format(
      nonce: nonce, Q: UInt32(messageLength), q: q, t: UInt8(tagLength),
  ...
  func encrypt(block plaintext: ArraySlice<UInt8>) -> [UInt8] {
    ...
    while processed < plaintext.count {
      ...
      let xored: [UInt8] = xor(
        plaintext[plaintext.startIndex.advanced(by: processed)...], keystream[keystreamPosIdx...])
  ...
  func decrypt(block ciphertext: ArraySlice<UInt8>) -> [UInt8] {
    ...
    do {
      ...
      while processed < ciphertext.count {
        ...
        let xored: [UInt8] = xor(
          ciphertext[ciphertext.startIndex.advanced(by: processed)...],
          keystream[keystreamPosIdx...])  // plaintext
        keystreamPosIdx += xored.count
        processed += xored.count
        output += xored
  ...
  func willDecryptLast(bytes ciphertext: ArraySlice<UInt8>) throws -> ArraySlice<UInt8> {
    ...
    return ciphertext[
      ciphertext.startIndex..<ciphertext.endIndex.advanced(
        by: -Swift.min(tagLength, ciphertext.count))]
  }
...
private func format(nonce N: [UInt8], Q: UInt32, q: UInt8, t: UInt8, hasAssociatedData: Bool) throws
  -> [UInt8]
{
  var flags0: UInt8 = 0

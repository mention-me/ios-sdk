...
public final class GCM: BlockMode {
  ...
  public init(
    iv: [UInt8], additionalAuthenticatedData: [UInt8]? = nil, tagLength: Int = 16,
    mode: Mode = .detached
  ) {
    self.iv = iv
  ...
  public convenience init(
    iv: [UInt8], authenticationTag: [UInt8], additionalAuthenticatedData: [UInt8]? = nil,
    mode: Mode = .detached
  ) {
    self.init(
      iv: iv, additionalAuthenticatedData: additionalAuthenticatedData,
      tagLength: authenticationTag.count, mode: mode)
    ...
    -> CipherModeWorker
  {
    ...
    let worker = GCMModeWorker(
      iv: iv.slice, aad: additionalAuthenticatedData?.slice, expectedTag: authenticationTag,
      tagLength: tagLength, mode: mode, cipherOperation: cipherOperation)
...
final class GCMModeWorker: BlockModeWorker, FinalizingEncryptModeWorker, FinalizingDecryptModeWorker
{
  ...
  init(
    iv: ArraySlice<UInt8>, aad: ArraySlice<UInt8>? = nil, expectedTag: [UInt8]? = nil,
    tagLength: Int, mode: GCM.Mode, cipherOperation: @escaping CipherOperationOnBlock
  ) {
...
private final class GF {
  ...
  func ghashUpdate(block ciphertextBlock: [UInt8]) -> UInt128 {
    ...
    x = GF.calculateX(
      block: addPadding(ciphertextBlock, blockSize: blockSize), x: x, h: h, blockSize: blockSize)
    return x
  }
  ...
  static func ghash(
    x startx: UInt128 = 0, h: UInt128, aad: [UInt8], ciphertext: [UInt8], blockSize: Int
  ) -> UInt128 {
  ...
  private static func calculateX(ciphertext: [UInt8], x startx: UInt128, h: UInt128, blockSize: Int)
    -> UInt128
  {
    ...
    for i in 0..<blocksCount {
      ...
      let block = pciphertext[
        pciphertext.startIndex.advanced(
          by: cpos)..<pciphertext.startIndex.advanced(by: cpos + blockSize)]
  ...
  private static func calculateX(
    block ciphertextBlock: [UInt8], x: UInt128, h: UInt128, blockSize: Int
  ) -> UInt128 {
  ...
  private static func calculateX(aad: [UInt8], x startx: UInt128, h: UInt128, blockSize: Int)
    -> UInt128
  {
    ...
    for i in 0..<blocksCount {
      ...
      let k =
        x
        ^ UInt128(
          paad[paad.startIndex.advanced(by: apos)..<paad.startIndex.advanced(by: apos + blockSize)])
      x = multiply(k, h)
    }

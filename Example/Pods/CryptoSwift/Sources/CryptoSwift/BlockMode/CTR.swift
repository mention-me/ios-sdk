...
public struct CTR: StreamMode {
    ...
    -> CipherModeWorker
  {
    ...
    return CTRModeWorker(
      blockSize: blockSize, iv: iv.slice, counter: counter, cipherOperation: cipherOperation)
  }
}
...
struct CTRModeWorker: StreamModeWorker, SeekableModeWorker, CounterModeWorker {
  ...
  init(
    blockSize: Int, iv: ArraySlice<UInt8>, counter: Int,
...
private func buildCounterValue(_ iv: [UInt8], counter: UInt64) -> [UInt8] {
  ...
  let nonceSuffix = iv[
    iv.startIndex.advanced(by: noncePartLen)..<iv.startIndex.advanced(by: iv.count)]

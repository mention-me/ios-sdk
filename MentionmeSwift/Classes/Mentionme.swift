...
public class Mentionme {
  ...
  /**
    ...
    */
  public func recordOrder(
    mentionmeOrderRequest: MentionmeOrderRequest,
    situation: String,
    ...
    guard let requestParameters = requestParameters else {
      noResponse(
        NSError(
          domain: "com.Mentionme.requestParameters.error", code: 405,
          userInfo: ["error": "There are no request parameters set."]) as Error)
      return
    }
  ...
  /**
     ...
     */
  public func enrolReferrer(
    mentionmeCustomerRequest: MentionmeCustomerRequest,
    situation: String,
    success: @escaping (
      _ mentionmeOffer: MentionmeOffer?, _ shareLinks: [MentionmeShareLink]?,
      ...
      success: { (data, response) in
        ...
        if let data = data {
          MentionmeParser.getOffer(
            ...
            failure: { (message) in
              DispatchQueue.main.async {
                noResponse(
                  NSError(
                    domain: "com.Mentionme.json.error", code: 405, userInfo: ["error": message])
                    as Error)
              }
            })
        } else {
          DispatchQueue.main.async {
            noResponse(
              NSError(
                domain: "com.Mentionme.json.error", code: 400,
                userInfo: ["error": "There was a problem with data"]) as Error)
          }
        }
  ...
  /**
     ...
     */
  public func getReferrerDashboard(
    mentionmeDashboardRequest: MentionmeDashboardRequest,
    situation: String,
    success: @escaping (
    ...
    performSessionDataTask(
      ...
      success: { (data, response) in
        ...
        if let data = data {
          MentionmeParser.getDashboard(
            data: data,
            success: { (offer, links, termsLinks, referralStats, dashboardRewards) in
              DispatchQueue.main.async {
  ...
  /**
     ...
     */
  public func findReferrerByName(
  ...
  /**
     ...
     */
  public func linkNewCustomerToReferrer(
    ...
    let request = mentionmeRefereeRegisterRequest.createRequest(
  ...
  public func entryPointForReferrerEnrollment(
    ...
    let request = mentionmeReferrerEnrollmentRequest.createRequest(
      requestParameters: requestParameters)
  ...
  private func performSessionDataTask(
    withRequest request: NSMutableURLRequest,
    success: @escaping (_ data: Data?, _ response: URLResponse?) -> Void,
    failure: @escaping (_ data: Data?, _ error: MentionmeError?) -> Void,
    noResponse: @escaping (_ error: Error?) -> Void
  ) {

    sessionDataTask = URLSession.shared.dataTask(with: request as URLRequest) {
      ...
      if let httpResponse = response as? HTTPURLResponse {
        ...
        if httpResponse.statusCode < 200 || httpResponse.statusCode > 299 {
          if let dataDict = MentionmeParser.getDictionary(data: data) {
            let mentionmeError = MentionmeError(
              withDataDictionary: dataDict, statusCode: httpResponse.statusCode)

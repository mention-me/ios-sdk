...
open class MentionmeValidationWarning {
  ...
  func validate(requestParameters: MentionmeRequestParameters) {
    ...
    do {
      ...
      let results = regex.matches(
        in: requestParameters.situation, options: [],
        range: NSRange(location: 0, length: requestParameters.situation.count))
  ...
  private func validateDashboardParameters(_ dashboardParameters: MentionmeDashboardParameters) {
    ...
    if let uniquerCustomerIdentifier = dashboardParameters.uniqueCustomerIdentifier,
      uniquerCustomerIdentifier.count > 255
    {
  ...
  private func validateReferrerByNameParameters(
    _ referrerByNameParameters: MentionmeReferrerNameParameters
  ) {
  ...
  private func validateCustomerParameters(_ customerParameters: MentionmeCustomerParameters) {
    ...
    if let segment = customerParameters.segment {
      ...
      do {
        ...
        let regex = try NSRegularExpression(pattern: pattern, options: [])
        let results = regex.matches(
          in: segment, options: [], range: NSRange(location: 0, length: segment.count))
